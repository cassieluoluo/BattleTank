	...
	public class Static_Track_CSEditor : Editor
	{
		...
		void Copy_To_Opposite (Transform	thisTransform, int type)
		{
			...
			if (oppositeTransform) {
				Static_Track_CS oppositeScript = oppositeTransform.GetComponent <Static_Track_CS> ();
				oppositeScript.Type = type; // Set Type.
				// Set Anchor.
				if (type == 1) {
					Static_Track_CS thisScript = thisTransform.GetComponent <Static_Track_CS> ();
		...
		void Update_ShadowMesh ()
		{
			...
			for (int i = 0; i < parentTransform.childCount; i++) {
				...
				if (childTransform.GetComponent <Static_Track_CS> ()) {
		...
		void Auto_Set_Others ()
		{
			...
			// Engine Sound
			Sound_Control_CS [] soundScripts = bodyTransform.GetComponentsInChildren <Sound_Control_CS> ();
			for (int i = 0; i < soundScripts.Length; i++) {
				if (soundScripts [i].Type == 0) { // Engine Sound.
					soundScripts [i].Track_Type = 1;
				}
			}
		}
		...
		bool Set_Closest_DrivingWheel (Transform pieceTransform, float radius)
		{
			...
			if (closestWheel) {
				Static_Track_CS pieceScript = pieceTransform.GetComponent <Static_Track_CS> ();
				pieceScript.Type = 1;
		...
		{ // If both sides of Anchor piece are Static, change to Dynamic.
			...
			Static_Track_CS frontScript = frontPiece.GetComponent <Static_Track_CS> ();
			Static_Track_CS rearScript = rearPiece.GetComponent <Static_Track_CS> ();
		...
		{ // called only in Parent Type.
			...
			for (int i = 0; i < parentTransform.childCount; i++) {
				Transform childTransform = parentTransform.GetChild (i);
				Static_Track_CS childScript = childTransform.GetComponent <Static_Track_CS> ();
				if (childScript) {
